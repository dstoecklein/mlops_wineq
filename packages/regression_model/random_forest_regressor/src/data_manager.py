...
from random_forest_regressor.config.core import DATA_PATH, ARTIFACTS_PATH, config
...
def _train_test_split(raw_data: pd.DataFrame) -> None:
    ...
    train_data.to_csv(Path(f"{DATA_PATH}/{config.app_config.train_data}"), index=False, sep=",")
    test_data.to_csv(Path(f"{DATA_PATH}/{config.app_config.test_data}"), index=False, sep=",")
